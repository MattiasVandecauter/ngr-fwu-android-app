...
- exposes the MCUmgr/SMP options in the app: window size, payload size, retries,
  and write-without-response mode
- defaults to the same MCUmgr/SMP framing, CBOR payloads, 384-byte chunks,
  10-request window, 3 retries, and write-with-response mode as the Python script
